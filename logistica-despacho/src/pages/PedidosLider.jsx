          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-3 pb-2 border-b border-slate-300">
            ...
            <div className="flex flex-col sm:flex-row w-full md:w-auto items-center gap-3">
              <button 
                ...
                className="bg-blue-600 text-white px-5 py-2.5 rounded-lg font-bold shadow-md hover:bg-blue-700 flex items-center justify-center gap-2 transition-colors w-full sm:w-auto"
              >
                <Plus size={18} /> Nuevo Despacho
              </button>

              <div className="flex w-full sm:w-auto items-center gap-2 bg-white p-1.5 md:p-2 rounded-lg shadow-sm border">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <input type="text" name="telefono" value={formData.telefono} onChange={handleChange} disabled={isReadOnly} required className="w-full border py-2.5 md:py-2 pr-3 pl-8 text-sm rounded disabled:bg-slate-100 bg-white"/>
                    ...
                    <div className="sm:col-span-2">
                      ...
                      <div className="relative">
                        <select name="destino_id" value={formData.destino_id} onChange={handleDestinoChange} disabled={isReadOnly} required className="w-full border py-2.5 md:py-2 pr-3 pl-8 text-sm rounded bg-white disabled:bg-slate-100">
                        ...
                        {formData.productos && formData.productos.length > 0 ? (
                          formData.productos.map((prod, index) => (
                            <tr key={index} className="hover:bg-slate-50">
                              ...
                              {habilitarRetiro && (
                                <td className="p-1 border"><input type="number" step="0.01" min="0" max={prod.cantidad} value={prod.cantidad_retirada_cliente || ''} onChange={(e) => handleProductoChange(index, 'cantidad_retirada_cliente', e.target.value)} disabled={formData.retiro_mostrador_entregado || (formData.id && formData.estado_entrega !== 'Pendiente')} className="w-full p-1 bg-orange-50 border border-orange-200 rounded text-center outline-none font-bold text-orange-600 focus:border-orange-400 disabled:bg-slate-100 disabled:border-slate-200 disabled:text-slate-400 disabled:cursor-not-allowed" placeholder="0" title="Cantidad retirada por el cliente en mostrador" /></td>
                              )}
                              {!isReadOnly && (
