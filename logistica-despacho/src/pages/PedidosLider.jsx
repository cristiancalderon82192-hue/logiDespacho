...
import { 
  ...
  Lock, Eye, AlertTriangle, XCircle 
} from 'lucide-react';
...
const PedidosLider = () => {
  ...
  const initialFormState = {
    ...
    valor_factura: '', fecha_facturacion: hoyLocal, 
  ...
  useEffect(() => {
    if (!user) return;
    fetchPedidos(true); 
    const intervalId = setInterval(() => { fetchPedidos(false); }, 5000); 
  ...
  return (
    <div className="bg-slate-50 min-h-screen p-3 md:p-8 w-full max-w-full overflow-x-hidden">
      <div className="max-w-7xl mx-auto space-y-6 md:space-y-8">
        ...
        <div className={`bg-white rounded-xl shadow-lg border overflow-hidden transition-all duration-300 ${isReadOnly ? 'border-slate-300' : editingId ? 'border-orange-400 ring-2 ring-orange-100' : 'border-slate-200'}`}>
          <div className={`px-4 md:px-6 py-3 md:py-4 flex items-center gap-3 border-b ${isReadOnly ? 'bg-slate-800 text-white' : editingId ? 'bg-orange-600 text-white' : 'bg-slate-900 text-white'}`}>
             {isReadOnly ? <Eye size={20} className="md:w-6 md:h-6" /> : editingId ? <Edit size={20} className="md:w-6 md:h-6"/> : <Truck size={20} className="md:w-6 md:h-6"/>}
             <div>
                <h2 className="text-sm md:text-xl font-bold">{isReadOnly ? `Viendo Pedido #${editingId}` : editingId ? `Editando Pedido #${editingId}` : 'Registrar Nuevo Despacho'}</h2>
                <p className="text-[10px] md:text-xs opacity-80">Líder de Sala</p>
             </div>
          </div>

          <form onSubmit={handleSubmit} className="p-4 md:p-6 grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8 animate-fadeIn">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
                <div className="sm:col-span-2">
                  ...
                  <div className="flex flex-col sm:flex-row gap-2">
                    ...
                    {!isReadOnly && <button type="button" onClick={() => setShowClientModal(true)} className="bg-blue-600 text-white py-2.5 md:py-2 px-4 rounded shadow-sm flex items-center justify-center gap-2 font-bold text-sm w-full sm:w-auto"><Search size={16} /> Buscar</button>}
                  </div>
                </div>
                <div>
                ...
                <div className="sm:col-span-2"><label className="text-[10px] md:text-xs font-bold text-slate-500 uppercase">Nota Manual</label><input type="text" name="nota_manual" value={formData.nota_manual} onChange={handleChange} disabled={isReadOnly} className="w-full border py-2.5 md:py-2 px-3 text-sm rounded disabled:bg-slate-100 bg-white" /></div>
              </div>
            </div>
            ...
            <div className="lg:col-span-2 bg-slate-50 p-3 md:p-4 rounded-lg border border-slate-200">
              ...
              <div className="grid grid-cols-4 md:grid-cols-8 gap-2 md:gap-3">
                {[1, 2, 3, 4, 5, 6, 7, 8].map((num) => (
                  <div key={num}><label className="block text-[9px] md:text-[10px] font-bold text-slate-400 uppercase text-center">B{num}</label><input type="number" name={`peso_b${num}`} value={formData[`peso_b${num}`]} onChange={handleChange} disabled={isReadOnly} className="w-full text-center py-2 md:py-1 px-1 border rounded focus:border-blue-500 font-bold text-sm text-slate-700 disabled:bg-slate-100 bg-white"/></div>
                ))}
              </div>
        ...
        {/* ================= TABLA DE MIS PEDIDOS CON SCROLL ================= */}
        <div className="space-y-4 md:space-y-6">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-3 pb-2 border-b border-slate-300">
             <div>
              <h2 className="text-xl md:text-2xl font-bold text-slate-800 flex items-center gap-2"><FileText className="text-slate-600" size={20}/> Mis Despachos</h2>
              <p className="text-[10px] md:text-xs text-slate-500 mt-1">Fecha: <span className="font-bold text-blue-600">{fechaFiltro}</span></p>
            </div>
            
            <div className="flex w-full md:w-auto items-center gap-2 bg-white p-1.5 md:p-2 rounded-lg shadow-sm border">
              <Calendar size={14} className="text-slate-400 ml-1"/>
              <input type="date" value={fechaFiltro} onChange={(e) => setFechaFiltro(e.target.value)} className="text-xs md:text-sm border-none outline-none text-slate-600 font-medium bg-transparent flex-1"/>
              <button onClick={() => fetchPedidos(true)} className="bg-blue-50 text-blue-600 p-2 md:p-1.5 rounded ml-1 transition-colors"><Search size={14} /></button>
            </div>
          </div>
              ...
              <table className="w-full text-left min-w-[750px]">
                ...
                <tbody className="divide-y divide-slate-100 text-xs md:text-sm">
                  {loading ? (
                    ...
                    <tr><td colSpan="7" className="p-8 text-center text-slate-400">No tienes pedidos agendados para esta fecha.</td></tr>
                  ) : (
                    pedidos.map((p) => {
                      const estaBloqueado = ['Asignado', 'En Ruta', 'Entregado', 'Entregado Incompleto', 'Devolución'].includes(p.estado_entrega);
                      ...
                      return (
                        <tr key={p.id} className={`transition-colors ${estaBloqueado ? 'bg-slate-50' : 'bg-white hover:bg-blue-50'}`}>
                          <td className="p-3 md:p-4 align-middle">
                            <p className="font-mono font-bold text-blue-600 text-sm md:text-base">{p.id_factura}</p>
                            <p className="text-[9px] md:text-[10px] text-slate-400 mt-0.5">{p.tipo_documento}</p>
                          ...
                          <td className="p-3 md:p-4 align-middle text-slate-600 font-medium">
                          ...
                          <td className="p-3 md:p-4 align-middle text-slate-800 font-medium flex items-center gap-1.5"><MapPin size={14} className="text-slate-400 shrink-0"/> {p.destino}</td>
