...
const AdminDashboard = () => {
  ...
  const [chartData, setChartData] = useState([]); // Estado para datos de la gráfica
  ...
  const fetchDashboard = async () => {
    ...
    try {
      ...
      // 3. TRANSFORMACIÓN DE DATOS PARA LA GRÁFICA
      // El backend devuelve { b1: 100, b2: 20 } -> Convertimos a array [{name:'B1', peso:100}, ...]
      if (data.bodegas) {
        const bodegasArray = [
  ...
  if (loading) return <div className="p-8 text-center text-slate-500">Cargando indicadores...</div>;
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row justify-between md:items-end gap-4 border-b border-slate-200 pb-6">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            Hola, <span className="font-bold text-slate-800">{user?.nombre_completo}</span>. Resumen de despachos filtrados por <span className="font-bold text-blue-500">Fecha Agendada</span>.
          </p>
        </div>
        
        {/* BARRA DE FILTRO DE FECHAS */}
        <div className="flex flex-wrap items-end gap-2 bg-white p-2 rounded-xl shadow-sm border border-slate-200">
          <div>
            ...
            <input 
              ...
              className="border border-slate-200 rounded-lg px-3 py-1 text-sm text-slate-700 outline-none focus:border-blue-500"
            />
          </div>
      ...
      {stats && stats.kpis && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100 flex items-center gap-4 transition-transform hover:scale-105">
            ...
            <div>
              <p className="text-slate-400 text-xs font-bold uppercase tracking-wider">Total Pedidos</p>
              <h3 className="text-3xl font-bold text-slate-800">{stats.kpis.total_pedidos}</h3>
              <p className="text-xs text-blue-400 font-medium mt-1">En el periodo seleccionado</p>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100 flex items-center gap-4 transition-transform hover:scale-105">
            ...
            <div>
              <p className="text-slate-400 text-xs font-bold uppercase tracking-wider">Kilos Despachados</p>
              <h3 className="text-3xl font-bold text-slate-800">
                {Number(stats.kpis.total_peso).toLocaleString('es-CO')} <span className="text-lg text-slate-400">kg</span>
              </h3>
              <p className="text-xs text-purple-500 font-medium mt-1">Peso acumulado</p>
            </div>
          </div>

          {/* Card 3: Valor Facturado */}
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100 flex items-center gap-4 transition-transform hover:scale-105">
            <div className="p-4 bg-green-50 text-green-600 rounded-lg">
              <TrendingUp size={32} />
            </div>
            <div>
              <p className="text-slate-400 text-xs font-bold uppercase tracking-wider">Valor Facturado</p>
              <h3 className="text-3xl font-bold text-slate-800">
                $ {Number(stats.kpis.total_valor).toLocaleString('es-CO')}
              </h3>
              <p className="text-xs text-green-500 font-medium mt-1">Facturación total</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
        <h3 className="text-lg font-bold text-slate-700 mb-6 flex items-center gap-2">
          <BarIcon className="text-purple-600"/> Distribución de Carga por Bodega
        </h3>
        <div className="h-80 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={chartData}>
              ...
              />
              <Bar dataKey="peso" radius={[4, 4, 0, 0]} barSize={50}>
              ...
              <tbody className="divide-y divide-slate-50">
                {stats.destinos && stats.destinos.length > 0 ? (
                  stats.destinos.map((d, index) => (
                    <tr key={index}>
                      <td className="px-6 py-3 font-medium text-slate-700">{d.destino}</td>
                      <td className="px-6 py-3 text-center text-slate-500">{d.entregas}</td>
                      <td className="px-6 py-3 text-right font-bold text-blue-600">
                        {Number(d.peso).toLocaleString()} kg
                      </td>
                    </tr>
                  ))
                ) : (
                  <tr>
                    <td colSpan="3" className="px-6 py-8 text-center text-slate-400">
                      No hay datos registrados en estas fechas.
                    </td>
                  </tr>
                )}
              </tbody>
                    ...
                    <div key={index}>
                      <div className="flex justify-between mb-1">
                        <span className={`text-sm font-bold ${
                          p.prioridad === 'Alta' ? 'text-red-500' : 
                          p.prioridad === 'Media' ? 'text-orange-500' : 'text-blue-500'
                        }`}>{p.prioridad}</span>
                      ...
                      <div className="w-full bg-slate-100 rounded-full h-2.5">
                        <div 
                          className={`h-2.5 rounded-full ${
                            p.prioridad === 'Alta' ? 'bg-red-500' : 
                            p.prioridad === 'Media' ? 'bg-orange-400' : 'bg-blue-400'
                          }`} 
                          style={{ width: `${(p.cantidad / stats.kpis.total_pedidos) * 100}%` }}
                        ></div>
                      </div>
                    </div>
                  ))}
